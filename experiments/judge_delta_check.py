...
step" survive the lock scenario, where CLIPSeg could not even be tested (a resolution
ceiling of appears_at_step=0.278 there, vs. the judge's 1.000 on these same images per
pilot/causal_relevance_results.csv)?
...
def score_from_lookup(conditions: pd.DataFrame, lookup: Dict[Tuple[int, str, int], int],
    ...
    undefined, not zero (the same convention used elsewhere in this project's delta scoring)."""
    rows = []
...
def main() -> None:
    ...
    ap.add_argument("--conditions-dir", default=None)
    ap.add_argument("--sbs-csv", default=None)
    ap.add_argument("--causal-relevance-csv", default=None)
    ap.add_argument("--out-csv", default="artifacts/judge_delta_results.csv")
    args = ap.parse_args()

    cfg = Config.from_yaml(args.config)
    repo_root = Path(__file__).resolve().parent
    args.conditions_dir = args.conditions_dir or str(repo_root / cfg.paths.pilot_conditions_dir)
    args.sbs_csv = args.sbs_csv or str(repo_root / cfg.paths.pilot_sbs_eval_csv)
    args.causal_relevance_csv = args.causal_relevance_csv or str(repo_root / cfg.paths.pilot_causal_relevance_csv)

...
ALPHA = _ANALYSIS_DEFAULTS.alpha
...
def clustered_check(df: pd.DataFrame) -> Dict[str, dict]:
    ...
    for cond in control_conditions(df):
        ...
        try:
            ...
            continue
        results[cond] = dict(p_value=p_value, n_groups=n_groups, significant=p_value < ALPHA,
...
def joint_threshold_topk_sweep(manifest: dict, cache_dir: Path, t_values=None, topk_values=None,
    ...
    Part C Step 5. `real vs shuffled` is fully RNG-independent;
    ...
    if t_values is None:
        _sweep = Config().scoring
        t_values = tuple(round(t, 2) for t in np.arange(
            _sweep.calibration_sweep_start, _sweep.calibration_sweep_stop, _sweep.calibration_sweep_step
        ))
    if topk_values is None:
        topk_values = _ANALYSIS_DEFAULTS.topk_sweep
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Analyze score_chains.py's results CSV")
    ...
    path = args.results_csv or str(cfg.paths.results_csv)
    main(path, args.manifest, args.cache_dir)

#!/usr/bin/env python
"""
Stage 3 of the pipeline: threshold -> delta masks -> IoU -> CSV. Pure numpy/scipy -- no diffusers, no torch, no
...
_DEFAULTS = Config().scoring

DEFAULT_SEED = _DEFAULTS.seed
...
# Value lives in config.py's ScoringConfig.threshold (config/default.yaml); this constant
# is kept for the many modules that already do `from score_chains import DEFAULT_THRESHOLD`.
DEFAULT_THRESHOLD = _DEFAULTS.threshold

# Pre-registered (Step 3): the four prompts skipped
...
CALIBRATION_PROMPT_IDS = frozenset(_DEFAULTS.calibration_prompt_ids)
HELD_OUT_PROMPT_IDS = frozenset(_DEFAULTS.held_out_prompt_ids)
...
def iou_top_k(attention_map: np.ndarray, delta_mask: np.ndarray, threshold_pct: float = _DEFAULTS.threshold_pct) -> float:
    """Ported from SpatialSemanticAlignment.phase_c_alignment_score (experiments/
...
def score_chains(manifest: dict, cache_dir: Path, threshold: float = DEFAULT_THRESHOLD,
                  seed: int = DEFAULT_SEED, threshold_pct: float = _DEFAULTS.threshold_pct) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Stage 3: threshold -> delta masks -> IoU -> CSV")
    ...
    out_csv = args.output or str(cfg.paths.results_csv)
    threshold = args.threshold if args.threshold is not None else cfg.scoring.threshold
    seed = args.seed if args.seed is not None else cfg.scoring.seed
    threshold_pct = args.threshold_pct if args.threshold_pct is not None else cfg.scoring.threshold_pct

    with open(manifest_path) as f:
        manifest_data = json.load(f)
    df = score_chains(manifest_data, Path(cache_dir_arg), threshold=threshold, seed=seed,
                       threshold_pct=threshold_pct)

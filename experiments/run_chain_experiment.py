...
Track 1 (Causal Relevance pilot, `pilot/`) found that CoIG's own compositional lock makes
a *persistence*-based faithfulness check blind: real chains and shuffled (wrong-step)
chains both showed persists_to_final = 0.83. Track 2 built two separate metrics to fix this:

  Metric A -- validated separately that SD1.5's cross-attention tracks correct
  attribute-to-subject binding in one-shot generation (OWL-ViT ground truth).

  Metric B (pilot/spatial_semantic_alignment.py) -- a Delta-Mask +
...
--------------------------------------------------------
The CR pilot's chains were generated by Gemini 2.5 Flash Image through Google's API -- a
closed model. Metric B needs to hook cross-
...
#
# Kaggle assigns either a T4 (sm_75) or a P100 (sm_60) with no way to pick one ahead of
# time. Kaggle's stock preinstalled torch has dropped sm_60 support, which crashes the
# P100 case with cudaErrorNoKernelImageForDevice. Pinning this exact dual-arch build
# supports both GPU types. diffusers/transformers are pinned high enough for CLIPSeg +
...
# =============================================================================
# MECHANISM_PROMPTS -- copied verbatim from metric A's validation set, already validated
# there: SD1.5 + seed 42 reliably renders the right number of detectable people for these
# 9 prompts. Reusing them (rather than writing new ones) means chain construction
# inherits that validation instead of gambling on untested prompts.
# =============================================================================
...
def token_indices(tokenizer, prompt: str, phrase: str) -> List[int]:
    ...
    The original single-token find_token_index (which only matches one exact token)."""

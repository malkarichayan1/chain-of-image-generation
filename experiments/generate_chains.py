#!/usr/bin/env python
"""
Stage 1 of the pipeline: prompts x seeds -> images + attention maps + manifest.json. Runs on GPU/Kaggle; pushed as
...
# =============================================================================
# CONFIG -- every value this single-file Kaggle kernel needs, gathered in one place.
# Deliberately NOT loaded from config.py/common (this file must stay single-file and
# import-free so it can be pasted into one Kaggle notebook cell) -- see module docstring.
# =============================================================================
...
GUIDANCE_SCALE = 7.5
MASK_PAD_FRAC = 0.15  # box_mask()'s default padding fraction around a detected subject box
...
def load_all_models() -> Models:
    ...
    txt2img = _load_pipeline(SD15_MODEL_CANDIDATES, StableDiffusionPipeline)
...
def box_mask(image: Image.Image, box: List[float], pad_frac: float = MASK_PAD_FRAC) -> Image.Image:
    w, h = image.size
    x0, y0, x1, y1 = box
    bw, bh = x1 - x0, y1 - y0
...
def build_and_persist_chain(prompt_id: int, seed: int, spec: dict, capture: AttentionCapture,
    ...
    for step_idx, (subject, attribute, _region) in enumerate(spec["pairs"], start=1):
        ...
        edited = models.txt2img(prompt=inpaint_prompt, num_inference_steps=NUM_INFERENCE_STEPS,
                                 guidance_scale=GUIDANCE_SCALE, generator=g, callback_on_step_end=_cb).images[0]

...
Per `docs/part-c-validation-design.md`, seven pre-registered checks run against the growth
...
`pilot/spatial_semantic_alignment.py`'s validated `SpatialSemanticAlignment` class, never
previously checked against it. `equivalence_check.py` confirms the two agree bit-for-bit across

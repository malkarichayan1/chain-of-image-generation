...
from config import CONDITION_PAIRS, DEFAULT_CONFIG_PATH, PilotConfig
...
def main() -> None:
    ...
    results_csv = args.results_csv or str(cfg.paths.causal_relevance_results_csv)
    ...
    if shuffled_persist > cfg.decision.shuffled_persist_min and substituted_persist < cfg.decision.substituted_persist_max:
        print(
            "  Shuffled persistence stays high while Substituted stays low -> "
            "the lock confound is visible. Continue to the full study."
        )
    elif (real_persist - substituted_persist) > cfg.decision.real_substituted_gap_min:
        print(

...
Supports
--------
- UNet-based pipelines (SD 1.5, SDXL) via diffusers AttnProcessor interface
- DiT-based pipelines (FLUX.1, SD 3) via register_forward_hook
"""
...
class SpatialSemanticAlignment:
    ...
    def phase_c_alignment_score(
        ...
        # Binarise top threshold_pct of attention activations
        threshold_val = np.percentile(composite_attention_map, 100.0 * (1.0 - threshold_pct))
        attn_bin = (composite_attention_map >= threshold_val)
...
def test_metric():
    ...
    assert iou_1 == 1.0, f"Expected 1.0, got {iou_1}"
    ...
    assert iou_3 < 0.2, f"Expected low IoU, got {iou_3}"

...
Adapted from the audited chain-of-image-generation pipeline's per-step judge,
duplicated here (rather than imported) so pilot/ has no runtime dependency on
that external pipeline's internal structure.
...
def _openrouter_ask_yes_no(image_path: str, question: str, max_retries: int = _DEFAULTS.openrouter_max_retries) -> str:
    ...
    if not api_key:
        ...
        return "error"
    model = os.getenv("OPENROUTER_JUDGE_MODEL", _DEFAULTS.openrouter_model)
    ...
    for attempt in range(max_retries):
        try:
            ...
            with _urlreq.urlopen(req, timeout=float(os.getenv("OPENROUTER_TIMEOUT", str(_DEFAULTS.openrouter_timeout)))) as resp:

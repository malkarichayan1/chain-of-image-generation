...
evaluate_sbs_images.py --all over the pilot's fixed images (one run, covering
every step x every ground-truth question) instead of asking new questions for
Real and Shuffled -- both conditions only relabel which step's already-collected
judgment corresponds to which claimed attribute. Substituted needs new judge
calls (see causal_relevance.py), since it asks about an attribute that was
never part of this chain's ground truth or question set.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--item_indexes", type=str, default=None)
...
def main() -> None:
    ...
    item_indexes_path = args.item_indexes or str(cfg.paths.item_indexes)
    seed = args.seed if args.seed is not None else cfg.selection.seed
    output_dir = args.output_dir or str(cfg.paths.conditions_dir)

    prompts_df = pd.read_csv(args.prompts_csv)
    sbs_eval_df = pd.read_csv(sbs_eval_csv)
    item_indexes = load_item_indexes(item_indexes_path)

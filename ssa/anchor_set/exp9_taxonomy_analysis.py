...
convolution -- the same "pooled ... Poisson-binomial over each row's own 1/n chance"
approach used elsewhere in this project, reimplemented here as a tested, reusable
primitive since no prior implementation exists anywhere in this repo.
...
_TAXONOMY_DEFAULTS = Config().taxonomy
_SIGNIFICANCE_DEFAULTS = Config().significance
EXPECTED_N_STEPS = _TAXONOMY_DEFAULTS.expected_n_steps  # NUM_INFERENCE_STEPS in taxonomy_capture_flux.py; windows assume this
EXP19_TOP_K = _TAXONOMY_DEFAULTS.exp19_top_k
SIGNIFICANCE_ALPHA = _SIGNIFICANCE_DEFAULTS.alpha
DEFAULT_REPRO_THRESHOLD = _SIGNIFICANCE_DEFAULTS.repro_threshold

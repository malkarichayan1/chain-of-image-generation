...
manifest's pinned seeds on every overlapping `prompt_id`) — see `CLAUDE.md`'s Workstream 3
...
- Part C's robustness battery (Holm-Bonferroni correction across the 5 experiments,
  ...
  (`pi_level_experiment/`) but **not yet against this FLUX data** — the p-values above are

...
Workstream 2's real, second-annotator-validated labels land.
...
def write_dummy_artifacts(out_dir: Path, seed: int = 0) -> None:
    ...
    print(f"Wrote {len(data['manifest']['images'])} synthetic images, {len(data['labels'])} "
          ...
          f"(SYNTHETIC DATA -- not a real result)")
...
def main() -> None:
    ...
    ap.add_argument("--seed", type=int, default=Config().seeds.dummy_seed)
    args = ap.parse_args()

...
Restricted to n >= 3 (CLAUDE.md / briefing §5.4): a derangement at n=2 has exactly one
possibility (the forced swap of the two attributes' maps), which makes permuted accuracy
1 - real_accuracy by arithmetic, not by any property of attention -- the same degenerate case
Experiment 3's design doc already rejected for within-item scrambling. n=2 images are skipped
entirely here, not partially scored.
...
MCNEMAR_SEED = 42  # matches exp3_attention_scramble.py's convention
